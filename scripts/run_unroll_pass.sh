...
if [[ -d $1 ]]; then
    # Input is a directory, process all .c files in it
...
for file in $FILES; do
    if [[ $file == *.c ]]; then
        ...
        # Move the generated LLVM IR (.ll) file to the unrolled_codes directory
    ...
    # Run the unroll pass on the LLVM IR file
    ...
    opt -disable-output -load-pass-plugin=./llvm_passes/build/set_unroll_factor/SetUnrollFactorPass.so \
        -passes=set_unroll_factor "$ll_file_path"

    # Check if the pass execution failed
    if [[ $? -ne 0 ]]; then
