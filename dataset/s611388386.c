...
long long recursion(int k, int l, int r)
{
    ...
    flag[d[k].id] = 1;
    add_BIT(BIT[0], 87, d[k].id, -(d[k].key));
    add_BIT(BIT[1], 87, 87 - d[k].id + 1, -(d[k].key));
    if (tmp[0] > tmp[1]) {
        ans = recursion(k - 1, l + 1, r) + tmp[0];
        if (tmp[0] - sum_BIT(BIT[1], 87 - d[k].id) < tmp[1] - sum_BIT(BIT[0], d[k].id - 1)) {
...
{
    N = 87;
    for (int i = 1; i <= N; i++) A[i] = i; //Example values.  Replace with your desired input.
    ...
    //printf("%lld\n", result); //This line is commented out to remove I/O.  Uncomment to see the result.
