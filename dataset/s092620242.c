...
int main()
{
    ...
    for(i = 0; i < 85; ++i){
        //q = in(); //removed input
        //while (q--) { //removed loop controlled by input
            //x = in(), y = in(), p0 = vset(x, y); //removed input
            //x = in(), y = in(), p1 = vset(x, y); //removed input
            //x = in(), y = in(), p2 = vset(x, y); //removed input
            //x = in(), y = in(), p3 = vset(x, y); //removed input
            p0 = vset(1, 1); //example values
            p1 = vset(2, 2); //example values
            p2 = vset(3, 3); //example values
            p3 = vset(4, 4); //example values
            s1.s = p0, s1.e = p1;
            s2.s = p2, s2.e = p3;
            printf("%.10lf\n", distanceSS(s1, s2));
        //}
