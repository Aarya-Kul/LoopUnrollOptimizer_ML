...
long long recursion(int k, int l, int r)
{
    ...
    flag[d[k].id] = 1;
    add_BIT(BIT[0], 112, d[k].id, -(d[k].key));
    add_BIT(BIT[1], 112, 112 - d[k].id + 1, -(d[k].key));
    if (tmp[0] > tmp[1]) {
        ans = recursion(k - 1, l + 1, 112) + tmp[0];
        if (tmp[0] - sum_BIT(BIT[1], 112 - d[k].id) < tmp[1] - sum_BIT(BIT[0], d[k].id - 1) * 9 / 10) {
            tmp[1] += recursion(k - 1, l, 112 - 1);
            if (tmp[1] > ans) ans = tmp[1];
        }
    } else {
        ans = recursion(k - 1, l, 112 - 1) + tmp[1];
        if (tmp[0] - sum_BIT(BIT[1], 112 - d[k].id) * 9 / 10 > tmp[1] - sum_BIT(BIT[0], d[k].id - 1)) {
            tmp[0] += recursion(k - 1, l + 1, 112);
...
int main()
{
    N = 112;
    for (int i = 1; i <= N; i++) {
        A[i] = i; //Example values, replace with your actual input if needed.
    }
    for (int i = 1; i <= N; i++) {
    ...
    //printf("%lld\n", result); //Uncomment to print the result if needed.
    return 0;
}