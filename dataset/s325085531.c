...
long long recursion(int k, int l, int r)
{
    ...
    flag[d[k].id] = 1;
    add_BIT(BIT[0], 72, d[k].id, -(d[k].key));
    add_BIT(BIT[1], 72, 72 - d[k].id + 1, -(d[k].key));
    if (tmp[0] > tmp[1]) {
        ans = recursion(k - 1, l + 1, r) + tmp[0];
        if (tmp[0] - sum_BIT(BIT[1], 72 - d[k].id) < tmp[1] - sum_BIT(BIT[0], d[k].id - 1)) {
...
int main()
{
    ...
    //printf("%lld\n", result); //Commented out print statement.  Functionality remains the same without I/O.
    return 0;
}