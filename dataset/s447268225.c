...
insert(struct node *p, int act, int index)
{
	if (p == NULL) {
		...
		if (act < p->activity) {
			...
			//printf("This case is not exist"); //Removed Printf
		}
	}
}
...
int main()
{
	...
	for (i = 0; i < N; i++) {
		//ret = scanf(" %ld", &A[i]); //Removed scanf
        A[i] = i; //Dummy value for A[i]
	...
	//printf("%ld\n",DP[ixend][iyend]); //Removed Printf
    //fflush(stdout); //Removed fflush
	free(A);
	free(Index);
	return EXIT_SUCCESS;
}