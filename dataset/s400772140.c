...
long long recursion(int k, int l, int r)
{
    ...
    if (tmp[0] > tmp[1]) {
        ...
        if (tmp[0] - sum_BIT(BIT[1], N - d[k].id) / 2 < tmp[1]) {
            tmp[1] += recursion(k - 1, l, r - 1);
            if (tmp[1] > ans) ans = tmp[1];
        }
    } else {
        ans = recursion(k - 1, l, r - 1) + tmp[1];
        if (tmp[0] > tmp[1] - sum_BIT(BIT[0], d[k].id - 1) / 2) {
...
int main()
{
    N = 109; //Fixed N to 109
    for (int i = 1; i <= N; i++) A[i] = i; //Initialized A with sample values
