...
void merge_sort(data x[], int n)
{
    ...
    int i, p, q;
    for (i = 0, p = 0, q = n/2; i < 62; i++) { // Changed to 62 iterations
        if (p >= n/2) y[i] = x[q++];
        else if (q >= n) y[i] = x[p++];
        else y[i] = (x[p].key < x[q].key)? x[p++]: x[q++];
    }
    for (i = 0; i < 62; i++) { // Changed to 62 iterations
        x[i] = y[i];
    }
}
...
{
    while (i <= 62) { // Changed to 62 iterations
        BIT[i] += k;
        i += (i & -i);
    }
}
...
long long recursion(int k, int l, int r)
{
    ...
    flag[d[k].id] = 1;
    add_BIT(BIT[0], 62, d[k].id, -(d[k].key)); 
    add_BIT(BIT[1], 62, 62 - d[k].id + 1, -(d[k].key));

    if (tmp[0] >= tmp[1]) {
        ans = recursion(k - 1, l + 1, r) + tmp[0];
        if (tmp[0] - sum_BIT(BIT[1], 62 - d[k].id) <= tmp[1] - sum_BIT(BIT[0], d[k].id - 1)) {
...
int main()
{
    N = 62; // Fixed N to 62
    for (int i = 1; i <= N; i++) {
        A[i] = i; //Dummy data for testing.  Replace with your actual data loading.
    }
    for (int i = 0; i < N; i++) {
        d[i].key = A[i+1];
        d[i].id = i + 1;
    ...
    //printf("%lld\n", result); //Commented out print statement.  Uncomment for testing.
