...
void solve()
{
	...
	dp[1][0] = p[1].ff * abs(1 - p[1].ss);
	dp[0][1] = p[1].ff * abs(n - p[1].ss);
	for(int i = 2; i <= n; i++) {
		for(int j = 0; j <= i; j++) {
			int k = i - j;
			if(j) {
				dp[j][k] = dp[j - 1][k] + p[i].ff * abs(j - p[i].ss);
			}
			if(k) {
				dp[j][k] = max(dp[j][k], dp[j][k - 1] + p[i].ff * abs(n - k + 1 - p[i].ss));
			}
		}
	}
	int ans = -inf;
