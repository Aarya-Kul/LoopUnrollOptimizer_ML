...
int main(){
    ...
    dp[0][0] = 0;
    dp[1][0] = a[0].first * abs(0 - a[0].second);
    dp[0][1] = a[0].first * abs(n-1 - a[0].second);

    for(i = 1; i <= n; i++){
        for(j = 1; j <= n; j++){
            dp[i][j] = max(dp[i-1][j] + a[i+j-1].first * abs(i-1 - a[i+j-1].second), 
                            dp[i][j-1] + a[i+j-1].first * abs(n-j - a[i+j-1].second));
        }
    }
